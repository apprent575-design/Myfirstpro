...
// --- Helper for Filters ---
const filterBookings = (bookings: Booking[], start?: string, end?: string, unitId?: string | string[], bookingIds?: string[]) => {
    return bookings.filter(b => {
        const isBookingMatch = !bookingIds || bookingIds.length === 0 || bookingIds.includes('all') || bookingIds.includes(b.id);
        const isUnitMatch = !unitId || (Array.isArray(unitId) ? (unitId.length === 0 || unitId.includes(b.unit_id)) : (unitId === 'all' || b.unit_id === unitId));
...
const formatVillage = (ar?: string, en?: string, lang?: string) => {
    if (!ar && !en) return lang === 'ar' ? 'القرية' : 'Village';
    if (lang === 'ar') {
        const name = ar || en;
        return name?.includes('قرية') ? name : `قرية ${name}`;
    }
    const name = en || ar;
    return name?.toLowerCase().includes('village') ? name : `${name} Village`;
};

export const generateReceipt = async (booking: Booking, unit: Unit | undefined, lang: Language, t: any) => {
    const isRTL = lang === 'ar';
    ...
    const labels = isRTL ? {
        ...
        status: 'حالة الحجز',
        payment: 'حالة الدفع',
        footer: `شكراً لاختياركم ${formatVillage(unit?.village_name_ar, unit?.village_name_en, lang)}.`
    } : {
        ...
        payment: 'Payment',
        footer: `Thank you for choosing ${formatVillage(unit?.village_name_ar, unit?.village_name_en, lang)}.`
    };
    ...
    <div style="font-family: 'Cairo', sans-serif; padding: 50px; color: #1f2937; max-width: 900px; margin: 0 auto; background: white; direction: ${isRTL ? 'rtl' : 'ltr'};">
      ...
      <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #0ea5e9; padding-bottom: 25px; margin-bottom: 35px;">
        <div style="display: flex; flex-direction: column;">
           <h1 style="color: #0284c7; font-size: 36px; margin: 0; font-weight: 800; letter-spacing: -0.5px;">${formatVillage(unit?.village_name_ar, unit?.village_name_en, lang)}</h1>
           <p style="color: #000000; font-size: 14px; margin: 5px 0; font-weight: 600;">Property Management</p>
        </div>
        ...
        <div style="flex: 1; background: #f8fafc; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0;">
            ...
            <div style="font-size: 15px;">
                <p style="margin: 8px 0; display:flex; justify-content:space-between;">
                    ...
                    <span style="font-weight:bold; color: #0369a1;">${unit?.name || ''}</span>
                </p>
                <p style="margin: 8px 0; display:flex; justify-content:space-between;">
      ...
      <div style="background: #fdf8f6; border: 2px dashed #fed7aa; color: #92400e; padding: 25px 20px; border-radius: 16px; margin-top: 25px; text-align: center;">
          <div style="font-size: 16px; font-weight: 700; margin-bottom: 8px;">
             ${lang === 'ar' ? 'يرجى التواصل مع ممثل المالك' : 'Please contact the property representative'} <strong style="color: #b45309; font-size: 20px; margin: 0 5px;">${booking.handler_name}</strong> 
          </div>
          <div style="font-size: 14px; font-weight: 600;">
             ${lang === 'ar' ? 'المسئول عن إجراءات الاستلام والتسليم للوحدة' : 'Responsible for unit check-in and check-out procedures'}
          </div>
          ${booking.handler_phone ? `
          <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #fde68a;">
             <span style="font-size: 15px; font-weight: bold; color: #78350f;">${lang === 'ar' ? 'رقم الهاتف :' : 'Phone Number :'}</span>
...
export const generateOccupancyReport = async (
    ...
    filterStart?: string,
    filterEnd?: string,
    filterUnitId?: string | string[],
    filterBookingIds?: string[]
) => {
    const isRTL = lang === 'ar';
    ...
    // Apply Filters
    const filteredBookings = filterBookings(bookings, filterStart, filterEnd, filterUnitId, filterBookingIds);
    // Filter Units based on bookings or selection
    const displayedUnits = units.filter(u => {
        if (filterUnitId && filterUnitId !== 'all') {
            if (Array.isArray(filterUnitId)) {
                return filterUnitId.length === 0 || filterUnitId.includes(u.id);
            }
            return u.id === filterUnitId;
        }
        return true;
    });
    ...
    // Group units by village
    const villageGroups: { [key: string]: Unit[] } = {};
    displayedUnits.forEach(u => {
        const vName = formatVillage(u.village_name_ar, u.village_name_en, lang) || (isRTL ? 'أخرى' : 'Other');
        if (!villageGroups[vName]) villageGroups[vName] = [];
        villageGroups[vName].push(u);
    });

    Object.entries(villageGroups).forEach(([villageName, vUnits]) => {
        let villageHtml = '';
        ...
        `}).join('');
            ...
            <tr style="background-color: #f1f5f9; border-top: 2px solid #cbd5e1; font-weight: bold;">
                <td colspan="3" style="padding: 12px; text-align: ${isRTL ? 'left' : 'right'}; color: #334155;">${labels.unitTotal}</td>
                <td style="padding: 12px; text-align: center; color: #0f172a; font-weight: 800;">${unitTotalNights}</td>
        ...
        if (villageHtml) {
            contentHtml += `
                <div style="margin-bottom: 40px;">
                    <h2 style="color: #0284c7; border-bottom: 3px solid #bae6fd; padding-bottom: 10px; margin-bottom: 20px; font-size: 22px; font-weight: 800;">
                        ${villageName}
                    </h2>
                    ${villageHtml}
                </div>
...
export const generateVillageFeesReport = async (
    ...
    Object.entries(villageGroups).forEach(([villageName, vUnits]) => {
        ...
        if (villageHtml) {
            contentHtml += `
                <div style="margin-bottom: 40px;">
                    <h2 style="color: #c2410c; border-bottom: 3px solid #fed7aa; padding-bottom: 10px; margin-bottom: 20px; font-size: 22px; font-weight: 800;">
                        ${villageName}
                    </h2>
                    ${villageHtml}
                </div>
...
export const generateSubscriptionReceipt = async (user: User, subscription: Subscription, lang: Language, t: any) => {
    ...
    const labels = isRTL ? {
        ...
        currency: 'ج.م',
        status: 'الحالة',
        footer: 'شكراً لاستخدامكم النظام.'
    } : {
        ...
        currency: 'EGP',
        status: 'Status',
        footer: 'Thank you for using the System.'
    };

    const html = `
    <div style="font-family: 'Cairo', sans-serif; padding: 50px; color: #1f2937; max-width: 900px; margin: 0 auto; background: white; direction: ${isRTL ? 'rtl' : 'ltr'};">
        <div style="text-align: center; border-bottom: 3px solid #0ea5e9; padding-bottom: 20px; margin-bottom: 40px;">
             <h1 style="color: #0284c7; font-size: 32px; font-weight: 800;">Property Management</h1>
...
export const generateExpenseReport = async (
    ...
    const filteredExpenses = expenses.filter(e => {
        const matchesUnit = !filterUnitId || filterUnitId === 'all' ||
            (Array.isArray(filterUnitId) ? (filterUnitId.length === 0 || filterUnitId.includes(e.unit_id)) : e.unit_id === filterUnitId);
        const matchesBooking = !filterBookingIds || filterBookingIds.length === 0 || filterBookingIds.includes('all') || (filterBookingIds.includes('none') && !e.booking_id) || (e.booking_id && filterBookingIds.includes(e.booking_id));
...
export const generateFinancialReport = async (
    ...
    // Filter Expenses
    const filteredExpenses = expenses.filter(e => {
        const isUnitMatch = !filterUnitId || filterUnitId === 'all' ||
            (Array.isArray(filterUnitId) ? (filterUnitId.length === 0 || filterUnitId.includes(e.unit_id)) : e.unit_id === filterUnitId);
        const isBookingMatch = !filterBookingIds || filterBookingIds.length === 0 || filterBookingIds.includes('all') || (filterBookingIds.includes('none') && !e.booking_id) || (e.booking_id && filterBookingIds.includes(e.booking_id));
        let isDateMatch = true;
        if (filterStart && filterEnd) {
            const eDate = parseISO(e.date);
            const rangeStart = startOfDay(parseISO(filterStart));
