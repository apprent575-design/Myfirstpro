...
export const AppProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const updateSystemSettings = async (settings: Partial<SystemSettings>) => {
    ...
    // Separate daily_email_time from other settings
    const { daily_email_time, ...otherSettings } = settings;
    
    // Call RPC to update cron schedule if time is provided
    if (daily_email_time) {
      const { error } = await supabase.rpc('update_daily_email_cron', { 
        time_string: daily_email_time 
      });
      if (error) throw error;
    }
    
    // Always update other settings if present
    ...
    // Refresh local state
    const { data } = await supabase.from('system_settings').select('*').eq('id', 'global').maybeSingle();
  ...
  const updateExpense = async (ex: Expense) => {
    ...
    if (!supabase) return;
    const { error } = await supabase.from('expenses').update(ex).eq('id', ex.id);
    if (error) throw error;
  };
