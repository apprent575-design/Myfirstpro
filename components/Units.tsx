...
export const Units = () => {
  const { t, state, addUnit, updateUnit, deleteUnit, language } = useApp();
  ...
  const [villageNameAr, setVillageNameAr] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        if (existing) {
          await updateUnit({ ...existing, name, type, village_name_ar: villageNameAr, village_name_en: villageNameEn });
          setEditingId(null);
        }
      } else {
        await addUnit({
  ...
  const startEdit = (unit: Unit) => {
    ...
    setName(unit.name);
    setType(unit.type);
    setVillageNameAr(unit.village_name_ar || '');
    setVillageNameEn(unit.village_name_en || '');
    setErrorMsg(null);
  };
                ...
                <div className="flex items-center gap-4 text-right">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 font-medium">
                      {t(unit.type.toLowerCase() as any)}
                      {unit.village_name_ar || unit.village_name_en ? ` • ${formatVillage(unit.village_name_ar, unit.village_name_en, language)}` : ''}
                    </p>
                  </div>
        ...
        <div className="w-full lg:w-[350px] order-1 lg:order-2 h-fit">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-[24px] shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold mb-1.5 text-gray-500 dark:text-gray-400 text-right">{language === 'ar' ? 'اسم القرية (عربي)' : 'Village Name (AR)'}</label>
                <input
                  type="text"
                  value={villageNameAr}
                  onChange={e => setVillageNameAr(e.target.value)}
                  className="w-full p-3 rounded-xl border border-gray-200 dark:border-slate-600 outline-none focus:ring-2 focus:ring-blue-500 text-right bg-white dark:bg-slate-700 text-gray-900 dark:text-white transition-colors"
                  placeholder={language === 'ar' ? 'مثال: عايدة' : 'e.g. Aida'}
                  dir="rtl"
                />
              </div>

              <div>
                <label className="block text-xs font-bold mb-1.5 text-gray-500 dark:text-gray-400 text-right">{language === 'ar' ? 'اسم القرية (انجليزي) اختياري' : 'Village Name (EN)'}</label>
                <input
                  type="text"
                  value={villageNameEn}
                  onChange={e => setVillageNameEn(e.target.value)}
                  className="w-full p-3 rounded-xl border border-gray-200 dark:border-slate-600 outline-none focus:ring-2 focus:ring-blue-500 text-left bg-white dark:bg-slate-700 text-gray-900 dark:text-white transition-colors"
      ...
      {unitToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm p-6 rounded-3xl shadow-2xl border border-gray-100 dark:border-slate-800">
            <h3 className="text-xl font-bold text-center mb-2 text-gray-900 dark:text-white" dir={language === 'ar' ? 'rtl' : 'ltr'}>
              {language === 'ar' ? 'حذف الوحدة؟' : 'Delete Unit?'}
            </h3>
            <p className="text-center text-gray-500 dark:text-gray-400 text-sm mb-6" dir={language === 'ar' ? 'rtl' : 'ltr'}>
              {language === 'ar' ? `سيتم حذف ${unitToDelete.name} نهائياً.` : `This will permanently remove ${unitToDelete.name}.`}
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={confirmDelete} disabled={isDeleting} className="flex-1 p-3 rounded-xl bg-red-500 text-white font-bold hover:bg-red-600">
