...
export const Expenses = () => {
  ...
  const filteredExpenses = state.expenses.filter(e => {
    const matchesUnit = filterUnitIds.length === 0 || filterUnitIds.includes(e.unit_id);
    const matchesBooking = filterBookingIds.length === 0 || filterBookingIds.includes('all') || (filterBookingIds.includes('none') && !e.booking_id) || (e.booking_id && filterBookingIds.includes(e.booking_id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        const originalExpense = state.expenses.find(ex => ex.id === editingId);
        await updateExpense({
          ...originalExpense,
          id: editingId,
          title: item.title,
          ...
          booking_id: expenseMeta.booking_id || null,
          created_at: originalExpense?.created_at || new Date().toISOString()
        } as unknown as Expense);
        // If added extra items during edit, add them too
        if (expenseItems.length > 1) {
          const promises = expenseItems.slice(1).map(extraItem => addExpense({
  ...
  const handleEdit = (expense: Expense) => {
    ...
    try {
      ...
      if (isValid(d)) validDate = format(d, 'yyyy-MM-dd');
    } catch (e) { }
    ...
    <div className="space-y-6 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-end md:items-center w-full md:w-auto">
            ...
            <FilterPopover>
              ...
              <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-100 dark:border-gray-700">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600">
                    ...
                    <input type="date" className="bg-transparent outline-none text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 w-full" value={mainBookingStart} onChange={e => setMainBookingStart(e.target.value)} title={language === 'ar' ? 'من تاريخ الحجز' : 'Booking from'} />
                    <span className="text-gray-400 font-bold shrink-0">-</span>
                    <input type="date" className="bg-transparent outline-none text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 w-full" value={mainBookingEnd} onChange={e => setMainBookingEnd(e.target.value)} title={language === 'ar' ? 'إلى تاريخ الحجز' : 'Booking to'} />
                  </div>
                  <MultiSelectBookings
                    bookings={state.bookings.filter(b => {
                      ...
                      if (isValid(d) && isValid(s) && isValid(e)) {
                        return (filterUnitIds.length === 0 || filterUnitIds.includes(b.unit_id)) &&
              ...
              <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-100 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600">
                  ...
                  <input
                    type="date"
                    className="bg-transparent outline-none text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 w-full"
                    value={filterStart}
          ...
          <div className="flex gap-2">
            <button
              onClick={() => generateExpenseReport(state.expenses, state.units, state.bookings, language, t, filterStart, filterEnd, filterUnitIds, filterBookingIds)}
