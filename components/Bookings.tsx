...
export const Bookings = () => {
  ...
  const [filterUnitIds, setFilterUnitIds] = useState<string[]>([]); // Multi Unit Filter
  ...
  const handleThisMonth = () => {
    ...
    setFilterEnd(format(endOfMonth(new Date()), 'yyyy-MM-dd'));
    setIsAllTime(false);
  };
  ...
  const handleAllTime = () => {
    ...
    state.bookings.forEach(b => {
      if (b.start_date) dates.push(parseDate(b.start_date).getTime());
      if (b.end_date) dates.push(parseDate(b.end_date).getTime());
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Enforce Village Name constraint
    const selUnit = state.units.find(u => u.id === (editingId ? state.bookings.find(b => b.id === editingId)?.unit_id || formData.unit_id : formData.unit_id));
    if (selUnit && !selUnit.village_name_ar && !selUnit.village_name_en) {
      setFormError(language === 'ar' ? 'لا يمكن إضافة/تعديل حجز لوحدة ليس لها اسم قرية. يرجى الذهاب لصفحة الوحدات وإضافة اسم القرية أولاً.' : 'Cannot save booking for a unit without a village name. Please update it in the Units page first.');
      return;
    }
  ...
  const isDefaultMonth = filterStart === format(startOfMonth(new Date()), 'yyyy-MM-dd') && filterEnd === format(endOfMonth(new Date()), 'yyyy-MM-dd');

  // Filter Bookings
  const filteredBookings = state.bookings.filter(b => {
    // 0. Bookings Identity Filter
    if (filterBookingIds.length > 0 && !filterBookingIds.includes('all') && !filterBookingIds.includes(b.id)) return false;

    // 1. Unit Filter
    if (filterUnitIds.length > 0 && !filterUnitIds.includes(b.unit_id)) return false;

    // 2. Date Filter
    if (!filterStart || !filterEnd) return true;
    const bookingDate = parseDate(b.start_date);
    if (!isValid(bookingDate)) return false;
    const start = parseDate(filterStart);
    const end = parseDate(filterEnd);
    if (!isValid(start) || !isValid(end)) return true; // Ignore filter if dates invalid
    return isWithinInterval(bookingDate, { start, end }) || bookingDate.getTime() === start.getTime() || bookingDate.getTime() === end.getTime();
  }).sort((a, b) => {
    const timeA = parseDate(a.start_date).getTime();
    const timeB = parseDate(b.start_date).getTime();
    ...
    const aFinished = aEnd.getTime() <= today.getTime();
    const bFinished = bEnd.getTime() <= today.getTime();

    if (aFinished && !bFinished) return 1;
    if (!aFinished && bFinished) return -1;
      
    // Sort all bookings ascending chronologically
    return timeA - timeB;
  });
            ...
            <FilterPopover>
              ...
              <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-100 dark:border-gray-700">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600">
                    ...
                    <input type="date" className="bg-transparent outline-none text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 flex-1 w-full" value={mainBookingStart} onChange={e => setMainBookingStart(e.target.value)} title={language === 'ar' ? 'من تاريخ الحجز' : 'Booking from'} />
                    <span className="text-gray-400 font-bold shrink-0">-</span>
                    <input type="date" className="bg-transparent outline-none text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 flex-1 w-full" value={mainBookingEnd} onChange={e => setMainBookingEnd(e.target.value)} title={language === 'ar' ? 'إلى تاريخ الحجز' : 'Booking to'} />
                  </div>
                  <MultiSelectBookings
                    bookings={state.bookings.filter(b => {
                      let show = true;
                      if (filterUnitIds.length > 0) {
                        show = filterUnitIds.includes(b.unit_id);
                      }
                      if (show && mainBookingStart && mainBookingEnd) {
                        const d = parseDate(b.start_date);
                        const s = parseDate(mainBookingStart);
                        const e = parseDate(mainBookingEnd);
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-2 py-1 text-[10px] font-bold bg-green-50 dark:bg-green-900/40 text-green-600 dark:text-green-400 rounded transition-colors shadow-sm"
                    >
                      {language === 'ar' ? 'اليوم / هذا الشهر' : 'This Month'}
                    </button>
                    <button
                      onClick={handleAllTime}
                      className="px-2 py-1 text-[10px] font-bold bg-primary-50 dark:bg-primary-900/40 text-primary-600 dark:text-primary-400 rounded transition-colors shadow-sm"
                    >
                      {t('allTime')}
                    </button>
                  </div>
                ...
                <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600">
                  ...
                  <input
                    type="date"
                    className="bg-transparent outline-none text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 flex-1 w-full"
                    value={filterStart}
                    onChange={(e) => { setFilterStart(e.target.value); setIsAllTime(false); }}
                    title="Start Date"
                  />
                  <span className="text-gray-400 font-bold shrink-0">-</span>
                  <input
                    ...
                    className="bg-transparent outline-none text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200 flex-1 w-full"
                    value={filterEnd}
                    onChange={(e) => { setFilterEnd(e.target.value); setIsAllTime(false); }}
                    title="End Date"
                  />
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-primary-50 dark:bg-primary-900/20 rounded-xl text-primary-700 dark:text-primary-300 flex justify-between items-center font-medium border border-primary-100 dark:border-primary-900/30">
                  <div>
                    ...
                    <input
                      type="date"
                      className="bg-transparent font-bold text-sm sm:text-base md:text-lg outline-none cursor-pointer w-[130px] sm:w-[150px] md:w-[180px] text-left pr-2"
                      value={formData.start_date}
                      onChange={e => setFormData({ ...formData, start_date: e.target.value })}
                    />
                  </div>
                  <div className="flex flex-col relative w-28 sm:w-36 border border-primary-200/50 dark:border-primary-900/50 rounded-xl overflow-hidden shadow-sm bg-white dark:bg-slate-900 shrink-0">
                    <span className="text-[10px] absolute top-1.5 left-0 w-full text-center font-bold text-gray-400">Time</span>
                    <input
                      type="time"
                      className="w-full h-full pt-5 pb-2 text-center bg-transparent text-sm font-bold focus:ring-inset focus:ring-2 focus:ring-primary-500 outline-none cursor-pointer"
                ...
                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-blue-700 dark:text-blue-300 flex justify-between items-center font-medium border border-blue-100 dark:border-blue-900/30">
                  <div>
                    ...
                    <div className="font-bold text-sm sm:text-base md:text-lg w-[130px] sm:w-[150px] md:w-[180px]">{formData.end_date}</div>
                  </div>
                  <div className="flex flex-col relative w-28 sm:w-36 border border-blue-200/50 dark:border-blue-900/50 rounded-xl overflow-hidden shadow-sm bg-white dark:bg-slate-900 shrink-0">
                    <span className="text-[10px] absolute top-1.5 left-0 w-full text-center font-bold text-gray-400">Time</span>
                    <input
                      type="time"
                      className="w-full h-full pt-5 pb-2 text-center bg-transparent text-sm font-bold focus:ring-inset focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer"
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        setFormData({
                          ...formData,
                          deposit_amount: val,
                          deposit_enabled: val > 0,
                          ...(val > 0 ? { status: BookingStatus.CONFIRMED } : {})
                        });
                      }}
                    />
                  </div>
