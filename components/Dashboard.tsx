...
import { isValid, addDays, isAfter, differenceInDays, isWithinInterval, startOfMonth, endOfMonth, format } from 'date-fns';
...
export const Dashboard = () => {
    ...
    const filteredBookings = state.bookings.filter(b => {
        ...
        const start = parseDateLocal(filterStart);
        const end = parseDateLocal(filterEnd);
        if (filterStart && filterEnd && isValid(start) && isValid(end)) {
            const bookingDate = parseDateLocal(b.start_date);
            if (!isValid(bookingDate)) return false;
            // Includes boundaries
            if (!isWithinInterval(bookingDate, { start, end }) && bookingDate.getTime() !== start.getTime() && bookingDate.getTime() !== end.getTime()) return false;
        }
    ...
    const activeBookings = state.bookings.filter(b => {
        ...
        const start = parseDateLocal(b.start_date);
    ...
    const netProfit = filteredBookings
    ...
    const rentals = filteredBookings
                    ...
                    <FilterPopover>
                        ...
                        <div className="flex flex-col gap-1.5 pt-2 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <div className="flex items-center gap-2 bg-gray-50 dark:bg-slate-700 p-2 rounded-lg border border-gray-200 dark:border-gray-600">
                                ...
                                <input
                                    type="date"
                                    className="bg-transparent outline-none text-[11px] font-bold text-gray-700 dark:text-gray-200 flex-1 min-w-[90px]"
                                    value={filterStart}
                            ...
                            {rentals.map((rental) => {
                                ...
                                return (
                                    <tr key={rental.id} className="hover:bg-blue-50/50 dark:hover:bg-blue-900/20 transition-colors group">
                                        ...
                                        <td className="p-5 text-center">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border shadow-sm ${rental.payment_status === 'Paid'
